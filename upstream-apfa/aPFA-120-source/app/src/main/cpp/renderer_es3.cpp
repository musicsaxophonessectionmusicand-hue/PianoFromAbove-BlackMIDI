// renderer_es3.cpp — RendererES3 impl (Android, OpenGL ES 3.0). See renderer_es3.h.
...
static const char* kNoteVS = R"(#version 300 es
layout(location=0) in vec2 aQuad;
layout(location=1) in float aStartSec;
...
out vec2 vSizePx;
void main() {
    int k = clamp(int(aKey + 0.5), 0, 127);
    vec2 kx = uKey[k];
    float ab = uKbFrac;
...
static const char* kGradVS = R"(#version 300 es
layout(location=0) in vec2 aQuad;        // 0,0 .. 1,1
layout(location=1) in vec4 aRect;        // x,y,w,h  normalised [0..1]
layout(location=2) in vec4 aCTL;         // top-left color
layout(location=3) in vec4 aCTR;         // top-right color
layout(location=4) in vec4 aCBR;         // bottom-right color
layout(location=5) in vec4 aCBL;         // bottom-left color
out vec4 vColor;
void main() {
...
static const char* kGradFS = R"(#version 300 es
precision mediump float;
in vec4 vColor;
out vec4 frag;
...
static const char* kSkewVS = R"(#version 300 es
layout(location=0) in vec2 aQuad;      // 0,0 .. 1,1
// per-instance corners: TL, TR, BR, BL (normalised 0..1 screen space)
layout(location=1) in vec2 aCTL;
layout(location=2) in vec2 aCTR;
layout(location=3) in vec2 aCBR;
layout(location=4) in vec2 aCBL;
// per-instance corner colours: TL, TR, BR, BL
layout(location=5) in vec4 aColTL;
layout(location=6) in vec4 aColTR;
layout(location=7) in vec4 aColBR;
layout(location=8) in vec4 aColBL;
out vec4 vColor;
void main() {
...
static const char* kSkewFS = R"(#version 300 es
precision mediump float;
in vec4 vColor;
out vec4 frag;
void main() { frag = vColor; }
...
static const char* kBgVS = R"(#version 300 es
layout(location=0) in vec2 aQuad;
...
static const char* kBgFS = R"(#version 300 es
precision mediump float;
in vec2 vTex;
...
void RendererES3::layoutKeyboard(int startNote, int endNote) {
...
void RendererES3::buildFontTexture() {
    glGenTextures(1, &fontTex_);
    glBindTexture(GL_TEXTURE_2D, fontTex_);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R8,
                 kAtlasW, kAtlasH, 0,
                 GL_RED, GL_UNSIGNED_BYTE, kFontAtlas);
...
bool RendererES3::buildPrograms() {
    noteProg_ = linkProgram(kNoteVS, kNoteFS);
    rectProg_ = linkProgram(kRectVS, kRectFS);
    gradProg_ = linkProgram(kGradVS, kGradFS);
...
bool RendererES3::resolveES3Entry() {
    pGenVertexArrays_     = (GenVertexArraysFn)     eglGetProcAddress("glGenVertexArrays");
    pBindVertexArray_     = (BindVertexArrayFn)     eglGetProcAddress("glBindVertexArray");
    pDrawArraysInstanced_ = (DrawArraysInstancedFn) eglGetProcAddress("glDrawArraysInstanced");
    pVertexAttribDivisor_ = (VertexAttribDivisorFn) eglGetProcAddress("glVertexAttribDivisor");
    return pGenVertexArrays_ && pBindVertexArray_ &&
           pDrawArraysInstanced_ && pVertexAttribDivisor_;
}

bool RendererES3::initEGL(void* window) {
    window_ = window;   // ANativeWindow* on Android
    ...
    // Bind the ES3-only entry points dynamically (VAOs + instancing). They are not
    // statically linked so libapfa.so stays loadable on ES2-only GPUs.
    if (!resolveES3Entry()) {
        LOGE("ES3 entry points unavailable (no real ES3 context)"); return false;
    }
    EGLint w = 0, h = 0;
    ...
    pBindVertexArray_(rectVao_);
    glBindBuffer(GL_ARRAY_BUFFER, quadVbo_);
    glEnableVertexAttribArray(0); glVertexAttribPointer(0,2,GL_FLOAT,GL_FALSE,8,(void*)0); pVertexAttribDivisor_(0,0);
    glBindBuffer(GL_ARRAY_BUFFER, rectInstVbo_);
    glEnableVertexAttribArray(1); glVertexAttribPointer(1,4,GL_FLOAT,GL_FALSE,20,(void*)0);  pVertexAttribDivisor_(1,1);
    glEnableVertexAttribArray(2); glVertexAttribPointer(2,4,GL_UNSIGNED_BYTE,GL_TRUE,20,(void*)16); pVertexAttribDivisor_(2,1);

    // --- gradient rect VAO --- stride=48 bytes (x,y,w,h + 4 colors)
    // GradInstance: 4 floats rect + 4*4bytes colors = 16+16=32 bytes
    pGenVertexArrays_(1, &gradVao_);
    pBindVertexArray_(gradVao_);
    glBindBuffer(GL_ARRAY_BUFFER, quadVbo_);
    glEnableVertexAttribArray(0); glVertexAttribPointer(0,2,GL_FLOAT,GL_FALSE,8,(void*)0); pVertexAttribDivisor_(0,0);
    glBindBuffer(GL_ARRAY_BUFFER, gradInstVbo_);
    // rect: x,y,w,h
    glEnableVertexAttribArray(1); glVertexAttribPointer(1,4,GL_FLOAT,GL_FALSE,32,(void*)0);  pVertexAttribDivisor_(1,1);
    // cTL, cTR, cBR, cBL
    glEnableVertexAttribArray(2); glVertexAttribPointer(2,4,GL_UNSIGNED_BYTE,GL_TRUE,32,(void*)16); pVertexAttribDivisor_(2,1);
    glEnableVertexAttribArray(3); glVertexAttribPointer(3,4,GL_UNSIGNED_BYTE,GL_TRUE,32,(void*)20); pVertexAttribDivisor_(3,1);
    glEnableVertexAttribArray(4); glVertexAttribPointer(4,4,GL_UNSIGNED_BYTE,GL_TRUE,32,(void*)24); pVertexAttribDivisor_(4,1);
    glEnableVertexAttribArray(5); glVertexAttribPointer(5,4,GL_UNSIGNED_BYTE,GL_TRUE,32,(void*)28); pVertexAttribDivisor_(5,1);

    // --- skew VAO --- SkewInstance: 8 floats (corners) + 4*4 bytes (colors) = 32+16=48 bytes
    pGenVertexArrays_(1, &skewVao_);
    pBindVertexArray_(skewVao_);
    glBindBuffer(GL_ARRAY_BUFFER, quadVbo_);
    glEnableVertexAttribArray(0); glVertexAttribPointer(0,2,GL_FLOAT,GL_FALSE,8,(void*)0); pVertexAttribDivisor_(0,0);
    glBindBuffer(GL_ARRAY_BUFFER, skewInstVbo_);
    // corners TL, TR, BR, BL — each vec2
    glEnableVertexAttribArray(1); glVertexAttribPointer(1,2,GL_FLOAT,GL_FALSE,48,(void*)0);  pVertexAttribDivisor_(1,1);
    glEnableVertexAttribArray(2); glVertexAttribPointer(2,2,GL_FLOAT,GL_FALSE,48,(void*)8);  pVertexAttribDivisor_(2,1);
    glEnableVertexAttribArray(3); glVertexAttribPointer(3,2,GL_FLOAT,GL_FALSE,48,(void*)16); pVertexAttribDivisor_(3,1);
    glEnableVertexAttribArray(4); glVertexAttribPointer(4,2,GL_FLOAT,GL_FALSE,48,(void*)24); pVertexAttribDivisor_(4,1);
    // colors TL, TR, BR, BL
    glEnableVertexAttribArray(5); glVertexAttribPointer(5,4,GL_UNSIGNED_BYTE,GL_TRUE,48,(void*)32); pVertexAttribDivisor_(5,1);
    glEnableVertexAttribArray(6); glVertexAttribPointer(6,4,GL_UNSIGNED_BYTE,GL_TRUE,48,(void*)36); pVertexAttribDivisor_(6,1);
    glEnableVertexAttribArray(7); glVertexAttribPointer(7,4,GL_UNSIGNED_BYTE,GL_TRUE,48,(void*)40); pVertexAttribDivisor_(7,1);
    glEnableVertexAttribArray(8); glVertexAttribPointer(8,4,GL_UNSIGNED_BYTE,GL_TRUE,48,(void*)44); pVertexAttribDivisor_(8,1);

    // --- text VAO ---
    pGenVertexArrays_(1, &textVao_);
    pBindVertexArray_(textVao_);
    glBindBuffer(GL_ARRAY_BUFFER, quadVbo_);
    glEnableVertexAttribArray(0); glVertexAttribPointer(0,2,GL_FLOAT,GL_FALSE,8,(void*)0); pVertexAttribDivisor_(0,0);
    glBindBuffer(GL_ARRAY_BUFFER, glyphVbo_);
    glEnableVertexAttribArray(1); glVertexAttribPointer(1,2,GL_FLOAT,GL_FALSE,16,(void*)0);  pVertexAttribDivisor_(1,1);
    glEnableVertexAttribArray(2); glVertexAttribPointer(2,1,GL_FLOAT,GL_FALSE,16,(void*)8);  pVertexAttribDivisor_(2,1);
    glEnableVertexAttribArray(3); glVertexAttribPointer(3,4,GL_UNSIGNED_BYTE,GL_TRUE,16,(void*)12); pVertexAttribDivisor_(3,1);

    // --- background-image VAO (just the unit quad at location 0) ---
    pGenVertexArrays_(1, &bgVao_);
...
void RendererES3::setNoteRange(int startNote, int endNote) {
    layoutKeyboard(startNote, endNote);
}

void RendererES3::setSwapInterval(bool vsync) {
    if (display_ != EGL_NO_DISPLAY)
        eglSwapInterval(display_, vsync ? 1 : 0);
}

void RendererES3::uploadBgImage(const uint8_t* rgba, int w, int h) {
...
void RendererES3::pushGradRect(float x, float y, float cx, float cy,
...
void RendererES3::pushFlatRect(float x, float y, float cx, float cy, uint32_t c) {
...
void RendererES3::pushSkew(float x1, float y1, float x2, float y2,
...
void RendererES3::flushGrads() {
    if (grads_.empty()) return;
    glBindBuffer(GL_ARRAY_BUFFER, gradInstVbo_);
    glBufferData(GL_ARRAY_BUFFER, grads_.size() * sizeof(GradInstance),
                 grads_.data(), GL_DYNAMIC_DRAW);
    glUseProgram(gradProg_);
    pBindVertexArray_(gradVao_);
    pDrawArraysInstanced_(GL_TRIANGLE_STRIP, 0, 4, (GLsizei)grads_.size());
    pBindVertexArray_(0);
    grads_.clear();
}

void RendererES3::flushSkews() {
    if (skews_.empty()) return;
    glBindBuffer(GL_ARRAY_BUFFER, skewInstVbo_);
    glBufferData(GL_ARRAY_BUFFER, skews_.size() * sizeof(SkewInstance),
                 skews_.data(), GL_DYNAMIC_DRAW);
    glUseProgram(skewProg_);
    pBindVertexArray_(skewVao_);
    pDrawArraysInstanced_(GL_TRIANGLE_STRIP, 0, 4, (GLsizei)skews_.size());
    pBindVertexArray_(0);
    skews_.clear();
}
...
void RendererES3::drawString(const char* str, float px, float py,
...
void RendererES3::flushGlyphs() {
    if (glyphs_.empty()) return;
    glBindBuffer(GL_ARRAY_BUFFER, glyphVbo_);
    glBufferData(GL_ARRAY_BUFFER,
                 glyphs_.size() * sizeof(GlyphInstance),
                 glyphs_.data(), GL_DYNAMIC_DRAW);
    ...
    pBindVertexArray_(textVao_);
    pDrawArraysInstanced_(GL_TRIANGLE_STRIP, 0, 4, (GLsizei)glyphs_.size());
    pBindVertexArray_(0);
    glyphs_.clear();
}
...
void RendererES3::renderKeyboard(const uint32_t keyColor[128]) {
...
void RendererES3::render(float clockSec, float totalSec, float fps,
    ...
    // Background (PFA: 0x80000000) — rect shader uses normalised [0..1] Y=0=bottom
    {
        ...
        rects_.push_back(bg);
        glBindBuffer(GL_ARRAY_BUFFER, rectInstVbo_);
        glBufferData(GL_ARRAY_BUFFER, sizeof(RectInstance), rects_.data(), GL_DYNAMIC_DRAW);
        glUseProgram(rectProg_);
        pBindVertexArray_(rectVao_);
        pDrawArraysInstanced_(GL_TRIANGLE_STRIP, 0, 4, 1);
    }
